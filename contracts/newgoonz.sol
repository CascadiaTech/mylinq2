...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  constructor(){
      ...
      rewardTokens.push(address(0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6)); // USDC - 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48
      //goerli - 0x07865c6E87B9F70255377e024ace6630C1Eaa37F ///goerli weth 0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6
...
contract Goonz is ERC20, Ownable {
    ...
    address private constant GOONNFT = 0x88Ba716d11BF03e657DE9b0eEe711F54ab9A411f;
    //IUniswapV2Router private router = IUniswapV2Router(UNISWAP_V2_ROUTER);
    IGoonNFT private nftcontract = IGoonNFT(GOONNFT);
   ...
   // uint public feeDivisor;
    ...
    constructor() ERC20("Goonz", "Goon") {

        uint totalSupply = (1000000) * (10**18);
        
        maxTransactionAmount = (1000000 * (10**18)) * (20 / 1000); // 1% maxTransactionAmountTxn
        swapTokensAtAmount = (1000000 * (10**18)) * (5 / 1000); // 0.05% swap tokens amount // set lower for testnet
        maxWallet = (1000000 * (10**18)) * (40 / 1000); // 4% Max wallet
        ...
        operationsBuyFee = 2;
        NFTBuyFee = 2;
        totalBuyFees = rewardsBuyFee + operationsBuyFee + NFTBuyFee;
        
        rewardsSellFee = 2;
        operationsSellFee = 2;
        NFTSellFee = 2;
        totalSellFees = rewardsSellFee + operationsSellFee + NFTSellFee;
    ...
    require(msg.sender == address(this) || msg.sender == owner(), "Caller is not the owner");
    _;
}

    event NFTRewardsAddded(uint ethtosend);
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _NFTFee) external onlyOwner {
        operationsBuyFee = _operationsFee;
        rewardsBuyFee = _rewardsFee;
        NFTBuyFee = _NFTFee;
        totalBuyFees = operationsBuyFee + rewardsBuyFee + NFTBuyFee;
        require(totalBuyFees <= 100, "Must keep fees at 10% or less");
    }
    
    function updateSellFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _NFTFee) external onlyOwner {
        operationsSellFee = _operationsFee;
        rewardsSellFee = _rewardsFee;
        NFTSellFee = _NFTFee;
        totalSellFees = operationsSellFee + rewardsSellFee + NFTSellFee;
    ...
    function _transfer(
        ...
        /// if the limits are in effect and if not from the owner too the owner, or adress 0 ordead, 
            if (
                from != owner() &&
                to != owner() &&
                to != address(0) &&
        ...
        if(takeFee){
            ...
            // on sell
                ...
                tokensForNFT += fees * NFTSellFee / totalSellFees;
            ...
            // on buy
                ...
                tokensForNFT += fees * NFTBuyFee / totalBuyFees;
    ...
    function sendETHToNFT(uint256 ethtosend) public onlyContract returns(bool)  {
        nftcontract.AddRewards{value: ethtosend}();
        emit NFTRewardsAddded(ethtosend);
        return true;
    }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForNFT + tokensForOperations + tokensForRewards;
        
        if(contractBalance == 0 || totalTokensToSwap == 0) {return;}
        
        // Halve the amount of liquidity tokens
        uint256 NFTTokens = contractBalance * tokensForNFT / totalTokensToSwap / 2;
        uint256 amountToSwapForETH = contractBalance.sub(NFTTokens);
        ...
        uint256 ethForOperations = ethBalance.mul(tokensForOperations).div(totalTokensToSwap - (tokensForNFT));
        uint256 ethForRewards = ethBalance.mul(tokensForRewards).div(totalTokensToSwap - (tokensForNFT));
        
        uint256 ethForNFT = ethBalance - ethForOperations - ethForRewards;
        
        tokensForNFT = 0;
        tokensForOperations = 0;
        tokensForRewards = 0;
        
        if(ethForNFT > 0 && NFTRewardsManual == false){
           sendETHToNFT(ethForNFT);
        }
